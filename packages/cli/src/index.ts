...
import {AdjudicationContract, DepositContract, CommitmentContract, ERC20Contract, OwnershipPayoutContract} from '@cryptoeconomicslab/tezos-contract'
import Cli from 'cac'
const cli = Cli()
...
async function instantiate() {
  const kvs = new LevelKeyValueStore(Bytes.fromString('tezos-liteclient-cli'), leveldown('.db'))
  ...
  const adjudicationContractAddress = TezosMessageUtils.writeAddress(process.env.ADJUDICATION_CONTRACT_ADDRESS as string)
  const commitmentContractAddress = TezosMessageUtils.writeAddress(process.env.COMMITMENT_CONTRACT_ADDRESS as string)
  if (!url) {
  ...
  const adjudicationContract = new AdjudicationContract(Address.from(adjudicationContractAddress), eventDb, wallet)
  const commitmentContract = new CommitmentContract(Address.from(commitmentContractAddress), eventDb, wallet)
  const erc20Contract = new ERC20Contract(Address.from(commitmentContractAddress), eventDb, wallet)
  const ownershipPayoutContract = new OwnershipPayoutContract(Address.from(commitmentContractAddress), eventDb, wallet)
  ...
  return new LightClient(wallet, kvs, adjudicationContract, depositContractFactory, tokenContractFactory, commitmentContract, ownershipPayoutContract, stateManager, syncManager, checkpointManager, config)
}

const depositContractAddress = TezosMessageUtils.writeAddress(process.env.DEPOSIT_CONTRACT_ADDRESS as string)
const tokenAddress = TezosMessageUtils.writeAddress(process.env.TOKEN_ADDRESS as string)
...
cli
  ...
  .action(async(mnemonic, email, password, address, options) => {
    const a = await TezosWalletUtil.unlockFundraiserIdentity(mnemonic, email, password, address)
